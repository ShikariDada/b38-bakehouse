...
export default async function CheckoutPage({ params, searchParams }: { params: Promise<{ code: string }>; searchParams: Promise<{ t?: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 sm:px-6 pt-[calc(var(--header-h)+2rem)] pb-24">
      <p className="eyebrow text-cocoa-600">Checkout</p>
      <h1 className="display display-md mt-2">Almost there</h1>
      <p className="num mt-2 text-[0.9rem] text-ink-soft">
        Order {order.public_code} · for {dateLabel}
      </p>

      <section className="mt-7 panel p-5">
        <h2 className="eyebrow text-cocoa-600">Your cake</h2>
        <dl className="num mt-3 space-y-1.5 text-[0.93rem]">
          ...
          <div className="rule pt-2.5 mt-2.5 flex justify-between font-medium">
      ...
      {needContact ? (
        <section className="mt-7">
          <h2 className="eyebrow text-cocoa-600">Who is this for?</h2>
          <form action={saveContact} className="mt-3 grid gap-4">
            ...
            <button className="btn btn-cocoa w-full sm:w-auto">Save details</button>
          </form>
        </section>
      ) : (
        <section className="mt-7">
          <h2 className="eyebrow text-cocoa-600">Paying</h2>
          ...
          <p className="mt-4 text-[0.82rem] text-ink-soft">
            Questions before paying? <a className="underline underline-offset-2" href={`https://wa.me/${s.whatsapp}?text=${encodeURIComponent(`Hi B38, I'm asking about order ${order.public_code}.`)}`} target="_blank" rel="noopener">Message us on WhatsApp</a>.
          </p>
        </section>
      )}
    </div>
